...
class MNIST_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, src_file):
        all_xy = np.loadtxt(src_file, usecols=range(785), delimiter="\t", comments="#", dtype=np.float32)
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Sigmoid(),
            nn.AvgPool2d(kernel_size=(2, 2), stride=1, padding=0),
        )
...
def main():
    ...
    train_data, valid_data = data.random_split(training_data,(48000,12000))
